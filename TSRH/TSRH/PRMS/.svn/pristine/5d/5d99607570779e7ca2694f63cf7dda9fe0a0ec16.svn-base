<!DOCTYPE html>
<%
  session.setAttribute("selectedLink","Reports"); 
%>
<html lang="en">
    <head>
        ...
        <script type="text/javascript">            
            ...
            {               
                ...
                //set min date
                $('#alertdate1').blur(function(){                    
                    var string = $('#alertdate1').val();
                    var e = string.split("/");                    
                    var datePicker = $("#alertdate2").data("kendoDatePicker");
                    var year = parseInt(e[2]);
                    var day = parseInt(e[1])+1;
                    var month = parseInt(e[0])-1;                            
                    if(string.length > 0){
                        datePicker.min(new Date(year, month, day));
                    }
                });
                
                //set max date
                $('#alertdate2').blur(function(){                    
                    var string = $('#alertdate2').val();
                    var e = string.split("/");                    
                    var datePicker = $("#alertdate1").data("kendoDatePicker");
                    var year = parseInt(e[2]);
                    var day = parseInt(e[1])-1;
                    var month = parseInt(e[0])-1;
                    if(string.length > 0){                    
                        datePicker.max(new Date(year, month, day));
                    }
                });
