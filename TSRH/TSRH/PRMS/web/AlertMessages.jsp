...
color: red; 
    ...
    <body id="page1" onLoad="upgradepicker()">
        <a href="javascript:$.pageslide({ direction: 'right', href: 'help/alertMessages.html' })" class="first">help</a>
        <div class="wrap">
            <!-- header -->
...
Date yest = cal.getTime();  
 ...
 <table class="bordered" >
	<strong>
        <tr>
			<th align='center'>Alert From</th>			
			<th align='center'>Alert Title</th>
...
for(int count = 0; count<alertinfo.length; count++) 
{
 ...
 %>	
      <tr>
          <td><%= alertinfo[count].getAlertCreatedBy()  %></td>  
          
          <%    if(alertinfo[count].getAlertTime().before(today)){
          %> 
 <td align='center'> <a class="red" href="alertdetail.jsp?alertId=<%= alertinfo[count].getAlertId() %>"  onClick="return popup(this, 'ExpandedView')">
            <%= alertinfo[count].getAlertSourceId()  %></a></td>
 <% }else{  %>
